    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                textPosition = getTextPosition(gameState, obsPos)
                ghostHouse = assignment[GHOST_HOUSE_VAR]==textPosition
                foodHouse = assignment[FOOD_HOUSE_VAR]==textPosition
                obsColor = assignment[obsVar]
                prob = 0

                if not ghostHouse and not foodHouse:
                    if obsColor == RED_OBS_VAL:
                        prob = 0
                    elif obsColor == BLUE_OBS_VAL:
                        prob = 0
                    elif obsColor == NO_OBS_VAL:
                        prob = 1           
                elif assignment[GHOST_HOUSE_VAR] == assignment[FOOD_HOUSE_VAR] or foodHouse:
                    if obsColor == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif obsColor == BLUE_OBS_VAL:
...
def getTextPosition(gameState,obsPos):
    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
    adjacentHouse=None
    distance=float("inf")
    for house in gameState.getPossibleHouses():
        tempDis=util.manhattanDistance(obsPos, house)
        if tempDis<distance:
            distance=tempDis
            adjacentHouse=house
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            ...
            leftValue, rightValue = self.computeEnterValues(newEvidence, enterEliminationOrder)
            expectedValue += max(leftValue, rightValue) * prob
