...
def depthFirstSearch(problem):
    ...
    explored={}
    frontier=util.Stack()
    #The first element in the node is its state,
    #The second element is its action,
    #The third element is its parent
    node=[problem.getStartState(),None,None]
    frontier.push(node)
    while not frontier.isEmpty():
        node=frontier.pop()
        state=node[0]
        if explored.has_key(hash(state)):
            continue
        explored[hash(state)]=True
        if problem.isGoalState(state):
            break;
        for successor in problem.getSuccessors(state):
            if not explored.has_key(hash(successor[0])):
                if problem.isGoalState(successor[0]):
                    frontier.push([successor[0],successor[1],node])
                    break
                frontier.push([successor[0],successor[1],node])
    solution=[]
    while node[1]!=None:
